...
export default function StudentsTable({ students, classes }: { students: StudentData[], classes: ClassData[] }) {
    ...
    return (
        <div className="space-y-6">
            {/* Toolbar */}
            <div className="flex flex-col xl:flex-row gap-4 justify-between items-start xl:items-center bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-card-border">
                {/* Search & Filter */}
                <div className="flex flex-col sm:flex-row gap-4 w-full xl:w-auto">
                    <div className="relative group w-full sm:w-72">
                        ...
                        <input
                            ...
                            className="w-full h-11 pr-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 text-sm font-bold focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all placeholder:text-muted-foreground/40"
                        />
                    </div>
                    <div className="relative group w-full sm:w-48">
                        ...
                        <select
                            ...
                            style={{ paddingLeft: '3.5rem' }}
                            className="w-full h-11 pr-8 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 text-sm font-bold focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all appearance-none cursor-pointer"
                        >
            ...
            {/* Table */}
            <div className="bg-white dark:bg-slate-900 rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-xl shadow-slate-200/40 dark:shadow-none min-h-[500px] flex flex-col">
                        ...
                        <tbody className="divide-y divide-card-border/50">
                            {paginatedStudents.length === 0 ? (
                                ...
                                paginatedStudents.map((s, index) => (
                                    <tr key={s.id} className={`group hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-colors ${selectedIds.includes(s.id) ? "bg-primary/5" : ""}`}>
                                        ...
                                        <td className="p-5">
                                            {editingId === s.id ? (
                                                ...
                                                <span className="px-3 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 text-[11px] font-black text-muted-foreground uppercase tracking-widest">{s.class.name}</span>
                                            )}
                                        </td>
                                                ...
                                                {editingId === s.id ? (
                                                    ...
                                                    <>
                                                        ...
                                                        <button
                                                            ...
                                                            className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-600/10 transition-all"
                                                        >
                                                            <Trash2 size={14} />
                                                        </button>
                                                    </>
                ...
                {/* Pagination Footer */}
                <div className="p-5 border-t border-card-border bg-slate-50/50 dark:bg-slate-800/50 flex flex-col sm:flex-row items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        <button
                            ...
                            disabled={page === 1}
                            className="p-2 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 disabled:opacity-30 disabled:hover:bg-transparent transition-all"
                        >
                            <ChevronLeft size={16} />
                        </button>
